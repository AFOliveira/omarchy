...
This directory contains a working native Arch RISC-V Omarchy desktop on the K3 cloud board, plus experimental kernel builds. The desktop runs in a persistent systemd-nspawn container on the Bianbu vendor kernel. It is not a replacement bootable Arch disk image. The upstream installer still assumes x86_64, Omarchy's package repositories, Limine, and a Btrfs root; use the board-specific staging scripts on Bianbu.
...
| Working host | Bianbu 4.0, vendor kernel `6.18.3-generic` |
...
- The restored Arch container reaches a healthy systemd state with no failed units. Its separate SSH listener uses keys only at `127.0.0.1:2223`; `omarchy-shell` on the Bianbu host opens the Arch user's shell.
- The vendor PowerVR userspace in `/opt/spacemit` initialized GBM, EGL 1.5, and OpenGL ES 3.2 on the BXM-4-64. Rendering requires DRM access through the container device policy.
...
- Native WayVNC streams Omarchy through the existing cloud viewer. The host proxy preserves the provider's original port 5901 and forwards to WayVNC bound only to `127.0.0.1:5900`. It replaces the stock WayVNC process, whose rendering path froze when connected to Hyprland.
...
- A clean container restart brings up the desktop and cloud stream automatically. The document portal has access to `/dev/fuse`; the system and user managers report no failed services.
- **A physical board reboot passed on 2026-09-05.** The boot ID changed, the vendor kernel returned, and the container, UWSM desktop, and cloud stream started automatically. `/boot/env_k3.txt` retained its SHA-256 digest across the reboot.
...
- BianbuCloud's power and system restart controls returned failures. Its stock-system recovery job succeeded after about eleven minutes; SSH returned with the vendor kernel. The factory recovery erased the first staged userspace. The native Arch system has since been restored and now starts through `omarchy-k3.service`.
...
## Native userspace staging

The current target is `/var/lib/machines/omarchy-k3`, alongside the Bianbu host. This is native RISC-V execution in a container, not a replacement bootable Arch installation. The Bianbu host retains the provider's remote access, boot chain, and GPU driver. Hyprland uses the stock Wayland session for its GPU allocator and creates an independent headless output for cloud rendering.
...
For the installed board, open the display panel in the BianbuCloud workbench. On an SSH connection to the Bianbu host, `omarchy-shell` opens the ordinary Arch user's shell. A separate key-only Arch SSH listener is available on host loopback port 2223. The local workspace's private SSH configuration also defines `omarchy-k3-arch` for direct access through the host.
...
- PipeWire and WirePlumber run, but the container currently exposes only a dummy audio output and no camera. Audio forwarding through the cloud viewer has not been implemented.
...
- Storage snapshots, Limine setup, physical suspend, and board firmware updates need a separate bare-metal installation design. The K3 updater therefore keeps the ported Omarchy source baseline and compositor ABI dependencies pinned.

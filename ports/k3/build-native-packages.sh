...
recipes=(aquamarine hyprland xdg-desktop-portal-hyprland yay omarchy-nvim ttf-ia-writer xdg-terminal-exec python-terminaltexteffects tobi-try elephant
...
for recipe in "${recipes[@]}"; do
  if [[ ! $recipe =~ ^[a-z][a-z0-9-]*$ ]] || [[ ! -f /run/omarchy-port/pkgbuilds/$recipe/PKGBUILD ]]; then
...
for recipe in "${recipes[@]}"; do
  ...
  cp -a /run/omarchy-port/pkgbuilds/"$recipe"/. "$build_dir/$recipe/"
  chown -R afonso:afonso "$build_dir/$recipe"
  (
    cd "$build_dir/$recipe"
    fingerprint=$(find "/run/omarchy-port/pkgbuilds/$recipe" -type f -print0 | sort -z | xargs -0 sha256sum | sha256sum)

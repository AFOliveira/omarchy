...
grep -Fx '  as_root /usr/bin/env OMARCHY_PATH=/usr/share/omarchy PATH="$root_path" \' <<<"$firewall_body" >/dev/null ||
...
grep -F 'local command_path="$package_path"' <<<"$user_omarchy_body" >/dev/null &&
  grep -F 'PATH="$command_path"' <<<"$user_omarchy_body" >/dev/null ||

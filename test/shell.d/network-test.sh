...
assert(/passwordIndex >= 0[\s\S]*credentialAbsenceTimer\.stop\(\)[\s\S]*else[\s\S]*passwordVisible = false[\s\S]*credentialAbsenceTimer\.restart\(\)/.test(wifiNetworksChanged[0]), 'network masks an unmounted prompt immediately and retains it only across bounded scan churn')
